...
class OpenAiProvider implements AiProviderInterface
{
    ...
    {
        $this->apiKey = $apiKey ?? (string) config('services.ai.openai.api_key', '');
        $this->model = $model ?? (string) config('services.ai.openai.model', 'gpt-4o-mini');
    ...
    public function gradeEssay(string $pertanyaan, ?string $indikator, string $rubrik, string $jawaban): array
    {
        if (empty($this->apiKey)) {
            throw new Exception('OpenAI API Key belum dikonfigurasi di server.');
        }

        if (empty($this->model)) {
            throw new Exception('Model AI belum dikonfigurasi di server.');
        }
...
PROMPT;
        ...
        try {
            ...
            ->retry($this->maxRetries, 1000, function ($exception, $request) {
                // Retry only on connection exceptions or 429/5xx status
            ...
            if ($response->failed()) {
                ...
                $apiMessage = $errorBody['error']['message'] ?? 'Unknown API error';

                Log::error('OpenAiProvider HTTP Error', [
                    'status' => $status,
                    'model' => $this->model,
                    'duration' => $duration,
                ...
                if ($status === 401) {
                    throw new Exception('Autentikasi OpenAI gagal: API key tidak valid.');
                } elseif ($status === 429) {
                    throw new Exception('OpenAI API rate limit / quota terlampaui. Silakan coba beberapa saat lagi.');
                } elseif ($status >= 500) {
                    throw new Exception('Server OpenAI sedang mengalami gangguan sementara (HTTP ' . $status . ').');
                } else {
                    throw new Exception('OpenAI API request gagal: ' . $apiMessage);
                }
            }
            ...
            if (empty($content)) {
                throw new Exception('Respons dari OpenAI kosong.');
            }

            $parsed = json_decode($content, true);
            if (!is_array($parsed)) {
                throw new Exception('Respons OpenAI bukan JSON yang valid.');
            }
