...
<div x-data="{ 
    ...
    <div class="space-y-4 mb-6">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div class="lg:col-span-4 bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
                ...
                <form action="{{ url('/app/admin/data-siswa') }}" method="GET" id="formFilterKelas">
            ...
            <div class="lg:col-span-4 bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
                ...
                <form action="{{ url('/app/admin/data-siswa') }}" method="GET" class="flex gap-2">
                    ...
                    @if(request('search') || request('kelas_id'))
                        <a href="{{ url('/app/admin/data-siswa') }}" class="btn-compact bg-slate-100 hover:bg-slate-200 text-slate-600 px-3 flex items-center justify-center min-h-[42px]" title="Reset Filter">✕</a>
                    @endif
                </form>
            </div>
            ...
            <div class="lg:col-span-4 bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex flex-col justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if($kelas_id)
                    <form action="{{ url('/app/admin/export-siswa') }}" method="GET" class="inline-block">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="py-3 px-4 text-center">
                            <div class="flex items-center justify-center gap-1.5">
                                {{-- Reset Password --}}
                                <form action="{{ url('/app/admin/data-siswa/'.$siswa->id.'/reset-password') }}" method="POST" onsubmit="return confirm('Reset username dan password siswa {{ addslashes($siswa->user->name ?? $siswa->nis) }} ke default NIS ({{ $siswa->nis }})?');">
                                    @csrf
                                ...
                                {{-- Hapus Siswa --}}
                                <form action="{{ url('/app/admin/data-siswa', $siswa->id) }}" method="POST" onsubmit="return confirm('Hapus siswa {{ addslashes($siswa->user->name ?? $siswa->nis) }} secara permanen?');">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="space-y-4 mb-8">
        <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm text-center">
            ...
            <p class="text-xs text-slate-500 mb-6 max-w-md mx-auto">Pilih salah satu kelas di bawah untuk melihat daftar siswa lengkap dalam 1 halaman penuh.</p>

            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3.5 max-w-4xl mx-auto">
                @foreach($kelas as $k)
                <a href="{{ url('/app/admin/data-siswa') }}?kelas_id={{ $k->id }}" 
    ...
    <div x-show="createModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl p-6 shadow-2xl w-full max-w-md" @click.outside="createModalOpen = false">
            ...
            <form action="{{ url('/app/admin/data-siswa') }}" method="POST" class="space-y-3.5">
                @csrf
                <div>
    ...
    <div x-show="editModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl p-6 shadow-2xl w-full max-w-md" @click.outside="editModalOpen = false">
            ...
            <form :action="`{{ url('/app/admin/data-siswa') }}/${editData.id}`" method="POST" class="space-y-3.5">
                @csrf
                @method('PUT')
    ...
    <div x-show="importModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl p-6 shadow-2xl w-full max-w-md" @click.outside="importModalOpen = false">
            ...
            <form action="{{ url('/app/admin/import-siswa') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
